...
import Author from "./src/models/author.mjs";
import Genre from "./src/models/genre.mjs";
import BookInstance from "./src/models/bookinstance.mjs";

import {
  connectToDatabase,
  disconnectFromDatabase,
} from "./src/lib/mongoose.mjs";
...
const genreCreate = async (index, name) => {
...
const bookCreate = async (index, title, summary, isbn, author, genre) => {
  const bookdetail = {
    title: title,
    summary: summary,
    author: author,
...
const bookInstanceCreate = async (index, book, imprint, due_back, status) => {
...
const createBookInstances = async () => {
  console.log("Adding authors");
  await Promise.all([
    bookInstanceCreate(
      0,
